...
I am now curious to see if the differences in growth between the baseline and control/treatments are actually significant, especially the [Fiber 2] + PCB group. To determine this I will preform an ANOVA as well as a Tukey significance test. 

Results from the ANOVA and Tukey test indicate that differences between all treatments and the baseline are significant with the exception of [Fiber 2] + PCB group (p = )
...
summary(aov(Growth$Shell_length~Growth$Treatment))

#according to the ANOVA there is a significant difference in at least one pair of the treatment groups..
# will now preform a Tukey test to determine where the difference(s) are

TukeyHSD(aov(Growth$Shell_length~Growth$Treatment))
